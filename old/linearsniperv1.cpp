// linearsniperv1.cpp
//
// Minimal, exact-landing v4 (output upgraded to block format):
//
// - NO energy caching.
// - NO secant slope.
...
//   --out_steps <path>  : per-iteration block trace file (one block per iteration)
...
// -----------------------------
// Arb printing helpers
// -----------------------------
static void fprint_arb_label_block(FILE* f, const char* label, const arb_t x, slong digits) {
    std::fprintf(f, "%s: ", label);
    arb_fprintn(f, x, digits, 0);
    std::fprintf(f, "\n");
}

static void print_arb_label_block(const char* label, const arb_t x, slong digits) {
    std::cout << label << ": ";
    arb_printn(x, digits, 0);
    std::cout << "\n";
}

// Compute log10(|x|) as an Arb interval, no zeta calls.
// If |x| is zero or contains nonpositive values, returns 0.
static void arb_log10_abs_safe(arb_t out, const arb_t x, const arb_t log10_const, slong prec) {
    arb_t ax;
    arb_init(ax);
    arb_abs(ax, x);
    ...
    // If ax could include 0 (wide interval), log is problematic; keep it simple and return 0.
    if (arb_contains_zero(ax)) {
        arb_zero(out);
        arb_clear(ax);
        return;
    }
    ...
    arb_log(out, ax, prec);
    arb_div(out, out, log10_const, prec);
...
// -----------------------------
// Iteration block printing
// -----------------------------
static void print_iter_block(
    FILE* f,
    int iter,
    int frac_after,
    const char* type_str,
    int step_size_eff,
    int probe_depth,
    const arb_t jump_midpm,
    const arb_t j_log10,
    const arb_t slope_midpm,
    const arb_t energy_midpm,
    const arb_t e_log10,
    const std::string& t_str,
    slong print_digits
) {
    std::fprintf(f, "ITER: %d\n", iter);
    std::fprintf(f, "FRAC: %d\n", frac_after);
    std::fprintf(f, "TYPE: %s\n", type_str);
    std::fprintf(f, "STEP_SIZE: %d\n", step_size_eff);
    std::fprintf(f, "PROBE: %d\n\n", probe_depth);

    std::fprintf(f, "JUMP(mid): ");
    arb_fprintn(f, jump_midpm, print_digits, 0);
    std::fprintf(f, "\n");

    std::fprintf(f, "J_LOG10: ");
    arb_fprintn(f, j_log10, print_digits, 0);
    std::fprintf(f, "\n");

    std::fprintf(f, "SLOPE(mid): ");
    arb_fprintn(f, slope_midpm, print_digits, 0);
    std::fprintf(f, "\n");

    std::fprintf(f, "ENERGY: ");
    arb_fprintn(f, energy_midpm, print_digits, 0);
    std::fprintf(f, "\n");

    std::fprintf(f, "E_LOG10: ");
    arb_fprintn(f, e_log10, print_digits, 0);
    std::fprintf(f, "\n");
...
static void print_iter_block_stdout(
    int iter,
    int frac_after,
    const char* type_str,
    int step_size_eff,
    ...
    std::cout << "ITER: " << iter << "\n";
    std::cout << "FRAC: " << frac_after << "\n";
    std::cout << "TYPE: " << type_str << "\n";
    std::cout << "STEP_SIZE: " << step_size_eff << "\n";
    std::cout << "PROBE: " << probe_depth << "\n\n";

    print_arb_label_block("JUMP(mid)", jump_midpm, print_digits);
    print_arb_label_block("J_LOG10", j_log10, print_digits);
    print_arb_label_block("SLOPE(mid)", slope_midpm, print_digits);
    print_arb_label_block("ENERGY", energy_midpm, print_digits);
    print_arb_label_block("E_LOG10", e_log10, print_digits);
...
int main(int argc, char **argv) {
    ...
    // Remove the OUT/OUT_STEPS/OUT_T banner at the top as requested.
    // (Paths will be printed in the final output block.)
    ...
    // Steps file will contain the per-iteration blocks
    std::fprintf(fsteps, "nsniperv4 iteration trace (block format)\n");
    std::fprintf(fsteps, "----------------------------------------\n");
    std::fflush(fsteps);
    ...
    arb_t log10_const, j_log10, e_log10;
    ...
    // log(10) constant for log10 conversion
    arb_const_log10(log10_const, prec_bits);
    ...
    while (total_steps < args.max_steps) {
        ...
        // Exact landing clamp (NEW in your v4):
        ...
        // Probe depth logic: DO NOT CHANGE
        ...
        // ---- Per-iteration block output (requested format) ----
        // TYPE is FIT vs CAP
        const char* type_str = is_cap ? "CAP" : "FIT";

        // J_LOG10 and E_LOG10 as intervals
        arb_log10_abs_safe(j_log10, safe_jump, log10_const, prec_step_bits);
        arb_log10_abs_safe(e_log10, e_now,     log10_const, prec_step_bits);

        // T line uses the updated t_work at the updated frac
        std::string t_iter_str = fixed_trunc_str_decimal_exact_cached(t_work, next_work_frac, p10);

        // Print digits: match your "big" style. Use (probe_depth + 256) but cap it to something sane.
        // You can change this to args.depth + 32 if you want everything enormous.
        slong print_digits = (slong)(probe_depth + 256);
        if (print_digits < 120) print_digits = 120;

        if (!args.quiet) {
            print_iter_block_stdout(
                total_steps, next_work_frac, type_str, step_size_eff, probe_depth,
                safe_jump, j_log10, slope, e_now, e_log10, t_iter_str, print_digits
            );
        }

        print_iter_block(
            fsteps, total_steps, next_work_frac, type_str, step_size_eff, probe_depth,
            safe_jump, j_log10, slope, e_now, e_log10, t_iter_str, print_digits
        );
        ...
        if (work_frac_digits >= args.depth) {
            // Final exact T and matching last energy reading at that T.
            ...
            // Ratio/drop vs the pre-step energy is no longer meaningful in this new per-iter format;
            // keep your existing final ratio/drop logic if you still want it.
            // Here we compute ratio vs current e_now (from the last iteration's pre-update energy),
            // which is at the last iteration's "pre" location.
            if (arb_is_zero(e_now)) {
                arb_zero(e_ratio);
                arb_zero(e_drop_pct);
            } else {
                arb_div(e_ratio, ctx.abs_z, e_now, prec_bits);
            ...
            // Final energy is computed at t_final with prec_bits (this is the "proper last energy reading").
            fprint_arb_label_block(fout, "FINAL SINK ENERGY (SCI, MID+/-RAD)", ctx.abs_z, 80);
    ...
    // max_steps reached without depth: final compute and log
