...
// -----------------------------
// Cheap log10 tokens for step trace
// -----------------------------
...
static void print_step_line(
    ...
    int n = std::snprintf(
        buf, sizeof(buf),
        "%4d frac=%-6d step=%-5d prec=%-5d %-3s dir=%s  E~%-10s  J~%-10s  S=%c\n",
        step_idx, frac_after_update, step_size_eff, prec_frac, mode, dch, eTok, jTok, sgn
    );
    if (n < 0) return;
...
static void print_arb_sci_line(const char* label, const arb_t x, slong digits) {
    std::cout << label << ": ";
    arb_printn(x, digits, 0);
    std::cout << "\n";
}

static void fprint_arb_sci_line(FILE* f, const char* label, const arb_t x, slong digits) {
...
int main(int argc, char** argv) {
    ...
    if (args.out_enabled) {
        ...
        std::cout << "OUT:       (disabled)\n";
        std::cout << "OUT_STEPS: (disabled)\n";
        std::cout << "OUT_T:     (disabled)\n";
    }
    ...
    std::cout << "STEP TRACE: step, frac(after), step_size, prec_frac, CAP/FIT, dir, log10(|E|), log10(|J|), slope sign\n";
    if (args.out_enabled) {
        std::fprintf(fsteps, "STEP TRACE: step, frac(after), step_size, prec_frac, CAP/FIT, dir, log10(|E|), log10(|J|), slope sign\n");
        std::fflush(fsteps);
    ...
    while (total_steps < args.max_steps) {
        ...
        if (!args.quiet) {
            print_step_line(total_steps, next_work_frac, step_size_eff, prec_frac, is_cap, dir_effective, e_now, safe_jump, slope);
        }
        if (args.out_enabled) {
            fprint_step_line(fsteps, total_steps, next_work_frac, step_size_eff, prec_frac, is_cap, dir_effective, e_now, safe_jump, slope);
        ...
        if (work_frac_digits >= args.depth) {
            // FINALIZATION: do NOT recompute HardyZ(+deriv) at the same T.
            // We already computed ctx.abs_z/ctx.hardy_z/ctx.hardy_zp for this iteration.
            // t_work has already been truncated to args.depth.
            ...
            std::string t_final_str = fixed_trunc_str_decimal_exact_cached(t_work, args.depth, p10);

            std::cout << ">>> DONE <<<\n";
            print_arb_sci_line("ENERGY", ctx.abs_z, 50);
            print_arb_sci_line("SLOPE ", final_slope, 50);
            std::cout << "T     : " << t_final_str << "\n";

            if (args.out_enabled) {
                std::fprintf(fout, ">>> DONE <<<\n");
                fprint_arb_sci_line(fout, "ENERGY", ctx.abs_z, 80);
                fprint_arb_sci_line(fout, "SLOPE ", final_slope, 80);
                std::fprintf(fout, "T     : %s\n", t_final_str.c_str());
                std::fflush(fout);
    ...
    // Max steps reached: report current state (one call here is unavoidable).
